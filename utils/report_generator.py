...
def preprocess_results(results, assessor_results):
    ...
    for method in res_per_m:
        res_per_gs_per_m[method] = {
            n: [r for r in res_per_m[method] if r["NumberOfAgents"] == n]
            for n in available_groupsizes
        }
        assessor_res_per_gs = {
            n: [r for r in assessor_results if r["NumberOfAgents"] == n]
            for n in available_groupsizes
        }
    ...
    if BRUTEFORCE_KEY in res_per_a_per_gs_per_m:
        ...
        for groupsize in res_per_a_per_gs_per_m[BRUTEFORCE_KEY]:
            honest_key = next(
                (
                    key
                    for key in res_per_a_per_gs_per_m[BRUTEFORCE_KEY][
                        groupsize
                    ].keys()
                    if "Honest" in key
                ),
            ...
            if res_per_a_per_gs_per_m[BRUTEFORCE_KEY][groupsize][honest_key]:
                uniqe_exp_id = list(
                    set(
                        [
                            r["experiment"]
                            for r in res_per_a_per_gs_per_m[BRUTEFORCE_KEY][
                                groupsize
                            ][honest_key]
                        ]
                    )
                ...
                for key in keys:
                    ...
                    best_result_per_gs[groupsize].append(
        ...
        for groupsize in res_per_a_per_gs_per_m[method]:
            ...
            sum_honest_results_per_groupsize[groupsize].append(
                parse_sum_data_entry(
                    method_avg_results[groupsize][honest_key],
            ...
            sum_dishonest_results_per_groupsize[groupsize].append(
                parse_sum_data_entry(
                    method_avg_results[groupsize][dishonest_key],
    ...
    avg_honest_results_per_measurement = {
        measure: {
            method: [
                ...
                if avg_results_per_method[method][groupsize][honest_key]
                else ""
    ...
    avg_assessor_results_per_measurement = {
        measure: {
            method: [
                ...
                if avg_results_per_method[method][groupsize][assessor_key]
                else ""
                for groupsize in avg_results_per_method[method]
            ]
            for method in avg_results_per_method
    ...
    avg_dishonest_results_per_measurement = {
        measure: {
            method: [
                ...
                if avg_results_per_method[method][groupsize][dishonest_key]
                else ""
                for groupsize in avg_results_per_method[method]
            ]
            for method in avg_results_per_method
    ...
    interval_dishonest_results_per_measurement = {
        measure: {
            method: [
                avg_results_per_method[method][groupsize][dishonest_key][
                    measure
    ...
    for groupSize in sum_honest_results_per_groupsize:
        if not sum_honest_results_per_groupsize[groupSize]:
            continue
        sum_honest_results_per_groupsize[groupSize].append(
            create_table_summary_line(
                sum_honest_results_per_groupsize[groupSize]
            )
        )
    ...
    for groupSize in sum_dishonest_results_per_groupsize:
        ...
        sum_dishonest_results_per_groupsize[groupSize].append(
            create_table_summary_line(
                sum_dishonest_results_per_groupsize[groupSize]
            )
        )
