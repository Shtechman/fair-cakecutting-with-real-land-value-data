...
# def generate_agents_to_file(meanValues,label,noise,numOfAgents):
#
#     output_path = "data/autoGenerated_%sAgents_%snoise_from_data_%s.txt" % (numOfAgents, noise, label)
#     print("creating %s agents for %s data with noise proportion %s" % (numOfAgents,label,noise))
#     agents = map(Agent, meanValues.noisyValuesArray(noise, None, numOfAgents))
#
#     print("saving agents to file %s" % output_path)
#     with open(output_path, "wb") as object_file:
#         pickle.dump(agents, object_file)
#
#     with open(output_path, "rb") as object_file:
#         agents = pickle.load(object_file)
#
#     print(agents)
#     return agents
...
def generate_valueMaps_to_file(original_map_file, folder, noise, num_of_maps, normalized_sum):

    with open(original_map_file) as data_file:
        original_map_data = json.load(data_file)

    index_output_path = "%s/index.txt" % folder
    print("Creating %s value maps for to folder %s with noise proportion %s" % (num_of_maps, folder, noise))
    ...
    for i in range(num_of_maps):
        ...
        with open(output_path, "wb") as object_file:
            pickle.dump(noisyValues(original_map_data, noise, normalized_sum), object_file)
        end = time.time()
...
if __name__ == '__main__':
    ...
    # nzMap = np.array(MapFileHandler.mapFromJson(nz2d), dtype=float)
    # # nzMap = np.array([[0.1]*10 for _ in range(10)],float)
    # rows = len(nzMap)
    # cols = len(nzMap[0])
    #
    # new_zMap = [[nzMap[i:i+2,j:j+2].sum() for j in range(0, cols, 2)] for i in range(0, rows, 2)]
    # with open('D:/MSc/Thesis/CakeCutting/data/newzealand_forests_2D_low_res.txt', 'w') as json_file:
    #     json.dump(new_zMap, json_file)
    ...
    folder = "D:/MSc/Thesis/CakeCutting/data/newZealandLowResAgents02"
    input_file = 'D:/MSc/Thesis/CakeCutting/data/newzealand_forests_2D_low_res.txt'
    # folder = "data/test02"
