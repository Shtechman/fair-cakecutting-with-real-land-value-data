    ...
    for testCsvMaps in testCsvMapFiles:
        ...
        cut_patterns_to_test = [
            ...
            for s in itertools.product(
                [CutDirection.Horizontal, CutDirection.Vertical], repeat=3
            )
        ]
        ...
        env = SimEnv(
            0, 0, agents, [], [], testResultPath, cut_patterns_to_test
        )
        results = []
        partitions = []
        ...
        for r, p in zip(results, partitions):
            direction_string = "%s,%s,%s" % (
                tuple(re.findall("Direction.([^:]*)", r["Method"]))
            )
